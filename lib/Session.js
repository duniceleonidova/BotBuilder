...
var Session = (function (_super) {
    ...
    function Session(options) {
        ...
        if (options && options.turbo_id) {
          this.setProcessingData({bot: options.turbo_id});
        }
    }
    Session.prototype.dispatch = function (sessionState, message) {
        if (message && message.text && message.from && message.from.channelId) {
            this.setProcessingData({
              user: this.userData.id || message.to.address,
              dialogChannelId: message.from.channelId == 'facebook' ?  message.from.address+'-'+message.to.address : this.channelData.id,
              message: message.text,
              platform: message.from.channelId,
              message_type : 'user-message'
          });
          eventBus.emit('send', this.processingData);
        }
        var _this = this;
        var index = 0;
        var handlers;
    ...
    Session.prototype.setProcessingData = function(data) {
        this.processingData = Object.assign({}, this.processingData, data);
    };
    return Session;
})(events.EventEmitter);
