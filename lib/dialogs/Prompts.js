            ...
            try {
                ...
                switch (args.promptType) {
                    ...
                    case PromptType.email:
                        var n = entities.EntityRecognizer.parseEmail(text);
                        if (n) {
                            var score = n.toString().length / text.length;
                            response = n;
                        }
...
var Prompts = (function (_super) {
            ...
            switch (style) {
                case ListStyle.button:
                    ...
                    if (session.dialogData.enumValues.length <= 3) {
                      ...
                      for (var i = 0; i < session.dialogData.enumValues.length; i++) {
                          var action = session.dialogData.enumValues[i];
                          a.push({ title: action, payload: action, content_type: "text" });
                      }
                      msg.setQuick(a)
                    }
                    break;
    ...
    Prompts.email = function (session, prompt, options) {
        var args = options || {};
        args.promptType = PromptType.email;
        args.prompt = prompt;
        beginPrompt(session, args);
    };
    ...
    Prompts.defaultRetryPrompt = {
        ...
        email: "I didn't recognize that as a email. Please enter a email."
    };
    return Prompts;
})(dialog.Dialog);
