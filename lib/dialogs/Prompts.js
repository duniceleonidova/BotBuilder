...
var Prompts = (function (_super) {
    ...
    Prompts.prototype.replyReceived = function (session) {
        ...
        if (checkStop(session.message.text)) {
          return session.endDialog({'stop': true});
        }
        ...
        if (retry && typeof args.retryPrompt === 'object' && !Array.isArray(args.retryPrompt)) {
            ...
            switch (style) {
                case ListStyle.button:
                    ...
                    if (session.dialogData.enumValues.length <= 3) {
                      var a = { actions: [] };
                      ...
                      var a = [];
                      for (var i = 0; i < session.dialogData.enumValues.length; i++) {
                          var action = session.dialogDsata.enumValues[i];
                          var cutAction = action;
                          if (action.length > 20) {
                           cutAction = action.slice(0, 17)
                           cutAction = cutAction + '...' 
                          }
                          a.push({ title: cutAction, payload: action.slice(0, 20), content_type: "text" });
                      }
                      msg.setQuick(a)
                    }
                    break;
    ...
    Prompts.defaultRetryPrompt = {
        ...
        choice: "I'm sorry, I didn't understand that. Please choose an option from the list or say \"stop\".",
...
function checkStop (text) {
  var stop_commands = ['cancel', 'nm', 'never mind', 'stop']
  text = text.toLowerCase().trim()
  return (stop_commands.indexOf(text) != -1)
}
